...
function ForgotPassword() {
  ...
  const handleSubmitClick = async (e: any) => {
    ...
    try {
      const result = await fetchFacad.postData<
        IForgotPasswordBody,
        IMessageBar
      >(`${SERVER}/password/forgot`, { email: email })
      console.log(result)
  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center text-center text-darkGrey'>
      ...
      <form
        className='text-gray-500 bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'
      ...
      >
        ...
        <div className='mb-4 text-left'>
          <input
            className={
              'shadow text-black appearance-none border focus:border-blueCegedim rounded w-full h-12 py-2 px-3 text-gray-700 leading-tight focus:border focus:outline-none focus:border-blue-500 focus:shadow-outline' +
          ...
          <p className={'text-red text-sm' + (!isInvalid ? ' hidden' : '')}>
            {errMessage.message.slice(0, 40) +
              (errMessage.message.length > 40 ? '...' : '')}
          </p>
        </div>
        <div className='flex items-center justify-center'>
          <input
            type='submit'
            className='transition-colors bg-blueCegedim w-full h-12 hover:bg-darkBlue text-white font-bold text-lg py-2 px-4 rounded focus:outline-none focus:shadow-outline cursor-pointer'
            value='Send Me Email'
          />
        </div>
